...
class InMemorySessionService(BaseSessionService):
  ...
  def update_session_parameters(
    ...
    if (
        app_name in self.sessions
        and user_id in self.sessions[app_name]
        and session_id in self.sessions[app_name][user_id]
    ):
        ...
        # self.sessions[app_name][user_id][session_id] is already updated by reference.
        # Return a deepcopy as per get_session's behavior.
        copied_session = copy.deepcopy(session)
        return self._merge_state(app_name, user_id, copied_session)
    return None
