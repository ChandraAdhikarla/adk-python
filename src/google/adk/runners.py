...
class Runner:
  ...
  def _register_agent_display_names(self, agent: BaseAgent, log_service: 'BaseLogService'):
    """Registers the agent and all sub-agents with the log service.
    
    This recursively walks through the agent tree and registers the display name
    for each agent with the log service. The display name is taken from the 
    agent.display_name attribute if it exists, otherwise the agent.name is used.
    
    Args:
        agent: The agent to register
        log_service: The log service to register with
    """
    # Check if agent has a name (it should always have one)
    if not agent.name:
        return
        
    # Check if display_name attribute exists, otherwise use name as both
    internal_name = agent.name
    display_name = agent.display_name if agent.display_name else agent.name
    
    # Log registration for debugging
    logger.debug(f"Registering agent name: {internal_name} -> {display_name}")
    
    # Register with log service
    log_service.register_agent_name(internal_name, display_name)
    
    # Register all sub-agents recursively
